function VAC_LR_hemisphere_2s_batch()
clear; clc;
%% =========================================================
% unified paths
%% =========================================================
...
dim2name = containers.Map( ...
    [REG_vel_dimK, REG_acc_dimK, REG_choice_dimK], ...
    {'VEL','ACC','CHOICE'} );
...
datasets = { ...
    struct('area','VIP',  ...
           ...
           'hemi','LR_hemisphere_VIP_2s.mat'), ...
    struct('area','MSTd', ...
           'mat','MSTd_Data_2s_REG.mat', ...
           'hemi','LR_hemisphere_MSTd_2s.mat') ...
    };
...
for d = 1:numel(datasets)
    ...
    fprintf('\n====================================================\n');
    fprintf('Running %d/%d: %s 2s\n', d, numel(datasets), ds.area);
    fprintf('====================================================\n');

    matfile  = fullfile(FilePath, ds.mat);
    hemifile = fullfile(FilePath, ds.hemi);

    if ~exist(matfile, 'file')
        warning('Data file not found: %s. Skip.', matfile);
        continue;
    end
    if ~exist(hemifile, 'file')
        warning('Hemisphere file not found: %s. Skip.', hemifile);
        continue;
    end

    run_one_dataset(matfile, hemifile, ds.area, ...
        window, T, REG_t, kernel, ...
        REG_params_kernel, REG_vel_dimK, REG_acc_dimK, REG_choice_dimK, ...
        id, axisNamesOrdered, nPerm, alphaPt, alphaCl, stim_type, durName);
...
%% =========================================================
function run_one_dataset(matfile, hemifile, areaName, ...
    window, T, REG_t, kernel, ...
    REG_params_kernel, REG_vel_dimK, REG_acc_dimK, REG_choice_dimK, ...
    id, axisNamesOrdered, nPerm, alphaPt, alphaCl, stim_type, durName)

load(matfile);
load(hemifile);   % loads LR_hemisphere

% hemisphere index
idxL = find(LR_hemisphere==1);
idxR = find(LR_hemisphere==2);
if isempty(idxL) || isempty(idxR)
    error('%s: LR_hemisphere must contain both 1 and 2.', areaName);
end
...
for ss = 1:length(stim_type)
    ...
    %% -----------------------------------------------------
    % Part 1: build trial-wise SDF per neuron for regression response
    %% -----------------------------------------------------
    ...
    for nCell = 1:size(REG_raster_per_trial,1)
        ...
        for i = 1:size(temp_spike,2)
            spike = squeeze(temp_spike(:,i,:)); % [ntrials x nsamples]
            [ntrials,~] = size(spike);
    ...
    %% -----------------------------------------------------
    % Part 2: VAC regression to get REG_b_use on ALL neurons
    %% -----------------------------------------------------
    ...
    for Ni = 1:size(Test_REG_r,1)
        ...
        TMP_Ves = (tempdata==1);   % keep original logic
    ...
    %% -----------------------------------------------------
    % Hemisphere-specific variance on SAME axes + significance
    %% -----------------------------------------------------
    fprintf('[%s] Hemisphere variance + significance...\n', stimname);
    ...
    % Plot
    %% -----------------------------------------------------
    figure('Color','w','Name',[areaName '_' stimname '_VAC_hemi_variance']);
    for k = 1:nAxes
        axName = axisNamesOrdered{k};
        ...
        title(sprintf('%s axis (VAC order): Left(solid) vs Right(dashed)', axName));
    ...
    %% -----------------------------------------------------
    OutHemi = struct();
    OutHemi.area     = areaName;
    OutHemi.duration = durName;
    OutHemi.stimname = stimname;
    ...
    SaveFileName = fullfile(OutputPath, sprintf('%s_%s_%s_%s.mat', areaName, durName, stimname, cell2mat(REG_params_use)));
    save(SaveFileName, 'OutHemi', '-v7.3');

    set(gcf,'paperpositionmode','auto');
    filename = fullfile(OutputPath, sprintf('%s_%s_%s_%s.png', areaName, durName, stimname, cell2mat(REG_params_use)));
