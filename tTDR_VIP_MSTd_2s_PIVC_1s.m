...
for s = 1:length(structures)
    clear REG_raster_per_trial REG_Stim REG_choice REG_VELangle REG_ACCangle
    if strcmp(structure_names{s}, 'VIP')
        matfile = fullfile(FilePath, 'VIP_Data_2s_REG.mat');
    elseif strcmp(structure_names{s}, 'MSTd')
    ...
    elseif strcmp(structure_names{s}, 'PIVC')
        ...
        error('Unknown structure: %s', structure_names{s});
    end
    ...
    for ss=1:length(stim_type)
        clear tempdata temp_mean
        clear Test_REG_r Test_PCA_Data REG_bVesK_Test
        clear projection Summary STPP SV KernelData
        ...
        for comb=1:size(b_comb,1)
            ...
            [colorStruct, haxP, vaxP, phR] = phaseSpace(projection, Summary, plotParams);
        ...
        OutputPath = 'Z:\Users\TDR\results\tTDR\';
